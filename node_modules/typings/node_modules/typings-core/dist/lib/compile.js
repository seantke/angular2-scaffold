...
function resolveFromWithModuleNameOverride(src, to) {
    if (typeof to === 'string') {
    ...
    if (options.browser && browser) {
        if (typeof browser === 'string') {
            ...
            for (var _i = 0, _a = Object.keys(browser); _i < _a.length; _i++) {
                var key = _a[_i];
                var from = resolveFromWithModuleNameOverride(tree.src, key);
                var to = resolveFromWithModuleNameOverride(tree.src, browser[key]);
                overrides[from] = to;
            }
        }
    }
...
function importPath(path, name, options) {
    ...
    if (path_3.isModuleName(resolved)) {
        ...
        return "" + prefix + config_1.DEPENDENCY_SEPARATOR + (modulePath ? path_3.fromDefinition(resolved) : resolved);
    }
    var relativePath = path_3.relativeTo(tree.src, path_3.fromDefinition(resolved));
...
function stringifyFile(path, rawContents, rawPath, options) {
    ...
    var moduleText = path_2.normalizeEOL(processTree(sourceFile, replacer, read), '\n');
    ...
    var modulePath = importPath(path, path_3.fromDefinition(path), options);
    var prettyPath = path_3.normalizeSlashes(path_1.join(name, path_3.relativeTo(tree.src, path_3.fromDefinition(path))));
